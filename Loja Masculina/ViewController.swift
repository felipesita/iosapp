...
//    }
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        LojaMasculina.webView.setNeedsLayout()
    }
    
    func initWebView() {
        LojaMasculina.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(LojaMasculina.webView);
        LojaMasculina.webView.uiDelegate = self;
        LojaMasculina.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    @objc func loadRootUrl() {
        ...
        LojaMasculina.webView.load(URLRequest(url: launchUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            LojaMasculina.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                LojaMasculina.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(LojaMasculina.webView.estimatedProgress);
    ...
    deinit {
        LojaMasculina.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: LojaMasculina.webView)
        }
